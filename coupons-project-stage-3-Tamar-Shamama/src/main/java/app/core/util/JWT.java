...
public class JWT {
		...
		@Value("${jwt.util.secret.key}")  // secret code in the resources
		...
		public boolean isTokenAdmin (String token) {
			String clientType = extractAllClaims(token).get("clientType").toString();
		...
		public int getClientId (String token) {
			...
			int clientId = extractAllClaims(token).get("clientId", Integer.class);
			return clientId;
